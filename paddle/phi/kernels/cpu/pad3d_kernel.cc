...
void Pad3dKernel(const Context& dev_ctx,
  ...
  int64_t in_depth = static_cast<int64_t>(in_dims[2]);
  int64_t in_height = static_cast<int64_t>(in_dims[3]);
  int64_t in_width = static_cast<int64_t>(in_dims[4]);
  int64_t out_depth = static_cast<int64_t>(out_dims[2]);
  int64_t out_height = static_cast<int64_t>(out_dims[3]);
  int64_t out_width = static_cast<int64_t>(out_dims[4]);
  ...
  const int64_t num = static_cast<int64_t>(in_dims[0]);
  if (data_format == "NCDHW") {
    std::map<std::string,
             void (*)(const T*,
                      T*,
