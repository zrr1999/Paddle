...
static void CalcGridLocationsWithGrad(const CPUContext& dev_ctx,
  ...
  const int64_t n = static_cast<int64_t>(grid.dims()[0]);
  const int64_t out_h = static_cast<int64_t>(grid.dims()[1]);
  const int64_t out_w = static_cast<int64_t>(grid.dims()[2]);
...
static void Calc3DGridLocationsWithGrad(const CPUContext& dev_ctx,
  ...
  const int64_t n = static_cast<int64_t>(grid.dims()[0]);
  const int64_t out_d = static_cast<int64_t>(grid.dims()[1]);
  const int64_t out_h = static_cast<int64_t>(grid.dims()[2]);
  const int64_t out_w = static_cast<int64_t>(grid.dims()[3]);
...
static void GatherOutputGradToInputGrad(const DenseTensor& output_grad,
  ...
  const int64_t n = static_cast<int64_t>(output_grad.dims()[0]);
  const int64_t c = static_cast<int64_t>(output_grad.dims()[1]);
  const int64_t out_h = static_cast<int64_t>(output_grad.dims()[2]);
  const int64_t out_w = static_cast<int64_t>(output_grad.dims()[3]);
  const int64_t in_h = static_cast<int64_t>(input_grad->dims()[2]);
  const int64_t in_w = static_cast<int64_t>(input_grad->dims()[3]);
...
static void Gather3DOutputGradToInputGrad(const DenseTensor& output_grad,
  ...
  const int64_t n = static_cast<int64_t>(output_grad.dims()[0]);
  const int64_t c = static_cast<int64_t>(output_grad.dims()[1]);
  const int64_t out_d = static_cast<int64_t>(output_grad.dims()[2]);
  const int64_t out_h = static_cast<int64_t>(output_grad.dims()[3]);
  const int64_t out_w = static_cast<int64_t>(output_grad.dims()[4]);
  const int64_t in_d = static_cast<int64_t>(input_grad->dims()[2]);
  const int64_t in_h = static_cast<int64_t>(input_grad->dims()[3]);
  const int64_t in_w = static_cast<int64_t>(input_grad->dims()[4]);
...
static void GatherBilinearGrad(const CPUContext& dev_ctx,
  ...
  const int64_t n = static_cast<int64_t>(grid_x->dims()[0]);
  const int64_t out_h = static_cast<int64_t>(grid_x->dims()[1]);
  const int64_t out_w = static_cast<int64_t>(grid_x->dims()[2]);
  const int64_t c = static_cast<int64_t>(input.dims()[1]);
...
static void Gather3DBilinearGrad(const CPUContext& dev_ctx,
                                 ...
                                 DenseTensor* grid_grad) {
  const int64_t n = static_cast<int64_t>(grid_x->dims()[0]);
  const int64_t out_d = static_cast<int64_t>(grid_x->dims()[1]);
  const int64_t out_h = static_cast<int64_t>(grid_x->dims()[2]);
  const int64_t out_w = static_cast<int64_t>(grid_x->dims()[3]);
  const int64_t c = static_cast<int64_t>(input.dims()[1]);
...
void GridSampleGradKernel(const Context& dev_ctx,
  ...
  if (x.dims().size() == 4) {
    ...
    const int64_t out_h = static_cast<int64_t>(grid.dims()[1]);
    const int64_t out_w = static_cast<int64_t>(grid.dims()[2]);
    const int64_t c = static_cast<int64_t>(x.dims()[1]);
    const int64_t in_h = static_cast<int64_t>(x.dims()[2]);
    const int64_t in_w = static_cast<int64_t>(x.dims()[3]);
    ...
    const int64_t out_w = static_cast<int64_t>(grid.dims()[3]);
    const int64_t c = static_cast<int64_t>(x.dims()[1]);
    const int64_t in_d = static_cast<int64_t>(x.dims()[2]);
    const int64_t in_h = static_cast<int64_t>(x.dims()[3]);
    const int64_t in_w = static_cast<int64_t>(x.dims()[4]);
